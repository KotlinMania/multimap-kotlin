...
Kotlin Multiplatform line-by-line clean-room port of the Rust crate [`multimap`](https://crates.io/crates/multimap).

> **Status: scaffold — porting has not started.** This repo currently contains build infrastructure only. The upstream Rust source for the `multimap` crate will be cloned into `tmp/` (gitignored) when porting begins.
...
```kotlin
dependencies {
    implementation("io.github.kotlinmania:multimap-kotlin:0.1.0-SNAPSHOT")
}
```
...
Original work copyrighted by the upstream `multimap` authors. Kotlin port copyright (c) 2026 Sydney Renee and The Solace Project.
