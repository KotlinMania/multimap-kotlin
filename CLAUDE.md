# Claude Code Project Instructions — multimap-kotlin

## Project Overview

This is **multimap-kotlin**, a clean-room Kotlin Multiplatform port of the upstream Rust crate [`multimap`](https://crates.io/crates/multimap).

The upstream Rust source is the read-only translation oracle. When porting begins, clone it into `tmp/multimap/` (gitignored). **Never edit `tmp/`.** If upstream looks wrong, the bug is in the port or in your understanding of Rust, not in `tmp/`.
...
```kotlin
// port-lint: source <path-relative-to-tmp/multimap>
