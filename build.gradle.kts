...
kotlin {
    ...
    swiftExport {
        moduleName = "Multimap"
        flattenPackage = "io.github.kotlinmania.multimap"
    }

    android {
        namespace = "io.github.kotlinmania.multimap"
        compileSdk = 34
        minSdk = 24
...
mavenPublishing {
    ...
    coordinates(group.toString(), "multimap-kotlin", version.toString())

    pom {
        name.set("multimap-kotlin")
        description.set("Kotlin Multiplatform port of havarnov/multimap - A multimap implementation")
        inceptionYear.set("2026")
        url.set("https://github.com/KotlinMania/multimap-kotlin")
        ...
        scm {
            url.set("https://github.com/KotlinMania/multimap-kotlin")
            connection.set("scm:git:git://github.com/KotlinMania/multimap-kotlin.git")
            developerConnection.set("scm:git:ssh://github.com/KotlinMania/multimap-kotlin.git")
        }
    }
}
    ...
    doFirst {
        ...
        if (sourceFiles.isEmpty()) {
            ...
            sentinelFile.writeText(
                ...
                package io.github.kotlinmania.multimap.codeql
...
// Make `./gradlew build` a strict full-target-surface gate: compile/link every
// configured target's main and test binary aggregator, every Android KMP
// compile/assemble path, and the registered XCFramework. The host-portable
// custom `test` task above only proves a runnable subset; `build` must prove
// that every configured target still links.
